...
$sqlTopicLikes = "SELECT like_id, topic_id, post_id, post_liked_by FROM likes WHERE topic_liked_by = '$id'";
$queryTopicLikes = mysqli_query($conn, $sqlTopicLikes);
while($topicLikes = mysqli_fetch_assoc($queryTopicLikes)){

  $topic_like_id = $topicLikes['topic_id'];

  $sql = "SELECT topic_id, topic_subject, topic_date, topic_cat, topic_type FROM topics WHERE topic_id = '$topic_like_id' ORDER BY topic_date DESC";
  ...
  while($topics = mysqli_fetch_assoc($query)){
    ...
    if($topic_type == 'Help'){
      $profileLikes .= "<h5 class='d-inline text-danger' id='post_type'> ".$topic_type."</h5>";
    }
    elseif($topic_type == 'Discussion'){
      $profileLikes .= "<h5 class='d-inline text-success' id='post_type'> ".$topic_type."</h5>";
    }
    elseif($topic_type == 'Other'){
      $profileLikes .= "<h5 class='d-inline text-dark' id='post_type'> ".$topic_type."</h5>";
    }
    ...
    $profileLikes .= "<h5 class='d-inline text-secondary'> &middot; ".$topic_cat_name."</h5>";
    $profileLikes .= "<p class='text-secondary mx-4'>".$topic_by_name." &middot; ".$topic_date_ago."</p>";
    $profileLikes .= "</a></li><hr>";
